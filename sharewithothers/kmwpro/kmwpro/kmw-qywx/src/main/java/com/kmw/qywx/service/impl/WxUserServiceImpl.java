...
public class WxUserServiceImpl implements IWxUserService {
	...
	/**
	 * 分页展示(可带条件查询) 企业微信用户信息信息 通用实体查询，Map参数 返回的是通用实体，不受实体属性限制，相当于map
	 * 
	 * @param params
	 * @return
	 */
	...
	/**
	 * 列表(可带条件查询) 企业微信用户信息信息 通用实体查询，Map参数 返回的是通用实体，不受实体属性限制，相当于map
	 * 
	 * @param params
	...
	/**
	 * 根据登录用户名得到用户信息放到实体中 从单点登录进入到系统中，不再判断用户名称密码
	 * 
	 *
	 */
	 ...
	 * 删除企业微信的一个用户信息
	 */
	public JSONObject deleteQywxUser(String userAccount) {
		String accessToken = WeiXinUtil.getAccessToken(WeiXinParamesUtil.corpId, WeiXinParamesUtil.agentSecret)
				.getToken();
//得到部门及子部门的人员信息
		JSONObject userJsonObject = contactsUserService.deleteUser(accessToken, userAccount);
	 ...
	 * 删除企业微信的一个用户信息
	 */
	public JSONObject addQywxUser(User user) {
		String accessToken = WeiXinUtil.getAccessToken(WeiXinParamesUtil.corpId, WeiXinParamesUtil.agentSecret)
				.getToken();
//得到部门及子部门的人员信息
		JSONObject userJsonObject = contactsUserService.createUser(accessToken, user);
		return userJsonObject;
	 ...
	 * 更新企业微信的一个用户信息
	 */
	public JSONObject updateQywxUser(User user) {
		String accessToken = WeiXinUtil.getAccessToken(WeiXinParamesUtil.corpId, WeiXinParamesUtil.agentSecret)
				.getToken();
//得到部门及子部门的人员信息
		JSONObject userJsonObject = contactsUserService.createUser(accessToken, user);
	...
	public int deleteAll(){
		logger.info("#=================删除表【企业微信用户信息】所有数据========================#");
		return  wxUserMapper.deleteWxUserAll();
	}
}
